...
int main(int argc, char *argv[])
{
    ...
    if (pos_flag == 1)
    {
        ...
        bi2.biWidth = (int)(bi2.biWidth * m);
        bi2.biHeight = (int)(bi2.biHeight * m);
        // bi2.biHeight *= n;
        ...
        // printf("\tb1\tb2\nHeight\t%d\t%d\nWidth\t%d\t%d\n", bi.biHeight, bi2.biHeight, bi.biWidth, bi2.biWidth);
        for (i = 0, biHeight = abs(bi2.biHeight); i < biHeight; i++)
        {
            // iterate over pixels in scanline
            // temporary storage
            RGBTRIPLE triple[bi2.biWidth];
            for (j = 0; j < bi2.biWidth; j++)
            {
                RGBTRIPLE temp;
                // fseek(inptr, sizeof(RGBTRIPLE), SEEK_CUR);
                fread(&temp, sizeof(RGBTRIPLE), 1, inptr);
                // printf("%d%d%d ", temp.rgbtRed, temp.rgbtGreen, temp.rgbtBlue);
                fseek(inptr, (freq - 1) * sizeof(RGBTRIPLE), SEEK_CUR);
                ...
                // }
                triple[j] = temp;
                // printf("added %d %d\t", i*freq, j*freq);
            ...
            // {
            // }
            fseek(inptr, (freq - 1) * (bi.biWidth * sizeof(RGBTRIPLE) + padding), SEEK_CUR);
            // for(j = 1; j < freq; j++){
